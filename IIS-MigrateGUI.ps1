...
$sourceProviders = @("iisApp", "contentPath", "package", "appHostConfig", "dirPath", "filePath")
$destProviders = @("auto", "iisApp", "contentPath", "package", "appHostConfig", "dirPath", "filePath")
...
foreach ($ctl in @(
        $cbVerb, $cbSrcProv, $cbDstProv, $txtSrcMain, $txtDstMain, $txtIP, $txtUser, $txtPass, $cbAuth,
        $lstFlags, $lstRules, $lstELinks, $lstDLinks
    )) {
...
$btnRun.Text = "RUN"
$btnRun.Location = "10,795"
$btnRun.Size = "120,30"
$btnRun.Add_Click({
        if ($cmdBox.Text.Trim()) {
            powershell -Command $cmdBox.Text
        }
    })
...
$btnDry.Size = "150,30"
$btnDry.Add_Click({
        $dryCmd = $cmdBox.Text + " -whatIf"
        powershell -Command $dryCmd
    })
